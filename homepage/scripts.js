...
const getNews = () => {
  fetch(NEWS_API_URL)
    ...
    .then((data) => {
      ...
      // Gunakan data.results bukan data.articles
      data.results.forEach(({ title, url, multimedia, published_date }) => {
        const clone = newTemplate.cloneNode(true);
        const li = clone.querySelector(".new");
        li.setAttribute("data-url", url);
        
        // Handle gambar dari multimedia array
        if (multimedia && multimedia.length > 0) {
          const image = document.createElement("img");
          image.classList.add("new__image");
          image.loading = "lazy";
          image.src = multimedia[0].url;
          image.alt = title;
          li.firstElementChild.prepend(image);
        }
        
        // Gunakan published_date bukan publishedAt
...
const handleSubmit = (e) => {
  ...
  const engine = window.defaultSearchEngine || "DuckDuckGo";
  let searchUrl = "";
  ...
  if (engine === "Google") {
    ...
    searchUrl = `https://search.yahoo.com/search?p=${encodeURIComponent(value)}`;
  } else {
